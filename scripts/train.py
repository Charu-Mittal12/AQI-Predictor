...
def load_data(horizon: int, training_cfg: dict) -> tuple:
    ...
    for batch in pf.iter_batches(batch_size=50_000, columns=keep_cols):
        ...
        # Downcast float64 → float32 immediately
    ...
    # Drop NaN targets, fill NaN features
    ...
    # Convert to numpy arrays (frees DataFrame overhead) 
    ...
    log.info(f"X_train shape: {X_train.shape} | "
             f"RAM: {X_train.nbytes/1e6:.1f} MB")
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray,
    ...
    horizon_metrics = {}
    for h in [1, 6, 12, 24]:
        idx = h - 1   # ← numpy index instead of .columns lookup
...
def get_model(name: str, params: dict):
    ...
    if name == "lgbm":
        ...
        raise NotImplementedError(
            "LSTM — Phase 2. Complete classical baselines first."
        )

    elif name == "transformer":
        raise NotImplementedError(
            "Transformer — Phase 2. Complete classical baselines first."
        )

    else:
        raise ValueError(
            f"Unknown model '{name}'. "
            f"Choose from: lgbm, xgboost, prophet, lstm, transformer"
        )
...
def main():
    ...
    log.info(f"Params    : {model_params}")
    log.info(f"{'='*60}")
